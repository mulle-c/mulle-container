...
void   *_mulle__uniquepointerarray_member2( struct mulle__uniquepointerarray *set, void *p)
{
   ...
   if( ! set->_sorted)
   {
      qsort( set->storage, _mulle__uniquepointerarray_get_count( set), sizeof( void *), (void *) _pointer_compare);
      set->_sorted = 1;
   }
   result = mulle__uniquepointerarray_bsearch( set->storage, _mulle__uniquepointerarray_get_count( set), p);
